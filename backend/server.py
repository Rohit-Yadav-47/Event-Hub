...
class Event(BaseModel):
    ...
    date: str  # Expecting format YYYY-MM-DD or ISO format
...
def convert_data_to_string(entities: List[dict], entity_type: str) -> str:
    ...
    if entity_type == "event":
        return "\n".join([
            f"Name: {e['name']}\nLocation: {e['location']}\nType: {e.get('type', e.get('category', ''))}\n"
            f"Date: {e.get('date', '')}\nTime: {e.get('time', '')}\n"
            f"Price: {e.get('price', '')}\nOriginal Price: {e.get('originalPrice', '')}\n"
            f"Category: {e.get('category', '')}\nOrganizer: {e.get('organizer', '')}\n"
            f"Attendees: {e.get('attendees', '')}\nTags: {', '.join(e.get('tags', []))}\n"
...
def get_day_from_date(date_input) -> str:
    """
    ...
    try:
        ...
        # Extract date string from dictionary if needed
        if isinstance(date_input, dict):
            # Try to find a date field in the dictionary
            if "date" in date_input:
                date_str = str(date_input["date"])
            else:
                logger.warning(f"Dictionary passed to get_day_from_date with no date field: {date_input}")
                return ""
        else:
        ...
        logger.warning(f"Failed to parse date: {date_input}, Error: {str(e)}")
        return ""
...
def get_entities_from_groq(query: str, entity_type: str) -> List[dict]:
    ...
    try:
        ...
        if entity_type == "event":
            fields = ["id", "name", "location", "type", "category", "date", "time", "description", "price", "originalPrice", "organizer", "attendees", "tags", "image_url"]
        elif entity_type == "user":
...
def format_entities_for_frontend(entities: List[dict], entity_type: str) -> List[dict]:
    ...
    for entity in entities:
        if entity_type == "event":
            ...
            if "T" in date:  # Handle ISO format dates
                date = date.split("T")[0]
            ...
            formatted.append({
                ...
                "type": entity.get("type", entity.get("category", "")),  # Fall back to category if type is missing
                ...
                "originalPrice": entity.get("originalPrice", None),
                "organizer": entity.get("organizer", ""),
                "attendees": entity.get("attendees", 0),
...
def ensure_event_dict_format(event_data):
    """
    Ensures event data is in dictionary format, converting from other formats if necessary.
    """
    # If event_data is already a dictionary, return it directly
    if isinstance(event_data, dict):
        return event_data
        
    # If it's a list, convert to dictionary (assuming position-based values)
    if isinstance(event_data, list):
        # Define expected event keys in order
        event_keys = ["id", "name", "date", "time", "location", "price", "originalPrice", 
                      "category", "image_url", "organizer", "attendees", "tags"]
        
        # Create dictionary from list based on position, using only available data
        return {key: event_data[i] if i < len(event_data) else None 
                for i, key in enumerate(event_keys)}
                
    # Return empty dict if format is unrecognized
    logger.error(f"Unrecognized event data format: {type(event_data)}")
    return {}
...
async def get_events(skip: int = 0, limit: int = 100):
    ...
    try:
        event_list = []
        for event_data in EVENTS[skip : skip + limit]:
            # Ensure the event is in dictionary format
            event_dict = ensure_event_dict_format(event_data)
            if event_dict:
                # Add derived day field
                event_dict["day"] = get_day_from_date(event_dict.get("date", ""))
                event_list.append(Event(**event_dict))
        return event_list
    except Exception as e:
...
async def get_event(event_id: int):
    ...
    try:
        ...
        event = None
        for e in EVENTS:
            e_dict = ensure_event_dict_format(e)
            if e_dict.get("id") == event_id:
                event = e_dict
                break
        ...
        # Add derived day field
        event["day"] = get_day_from_date(event.get("date", ""))
        return Event(**event)
